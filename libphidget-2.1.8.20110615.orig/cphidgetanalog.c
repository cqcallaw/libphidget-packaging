...
CPHIDGETDATA(Analog)
	...
	if(tsd && !phid->lastTsd)
	{
		if (phid->phid.fptrError)
		{
			snprintf(error_buffer,sizeof(error_buffer),"Thermal shutdown detected. All outputs have been disabled.");
	...
	else if(!tsd && phid->lastTsd)
	{
		if (phid->phid.fptrError)
		{
			snprintf(error_buffer,sizeof(error_buffer),"Thermal shutdown state ended.");
			phid->phid.fptrError((CPhidgetHandle)phid, phid->phid.fptrErrorptr, EEPHIDGET_OK, error_buffer);
		}
	}
	phid->lastTsd = tsd;
