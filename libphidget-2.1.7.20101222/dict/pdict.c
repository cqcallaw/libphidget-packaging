...
pdict_add(pdict_t *pd, const char *k, const char *v, const char **ovp)
{
	...
	if (ptree_contains((void *)&k, pd->pd_ents, pdecmp, (void *)&n)) {
		...
		ov = n->pde_val;
		n->pde_val = v;
		_pdict_ent_notify(n, PDR_VALUE_CHANGED, ov);
		if (ovp)
			*ovp = ov;
		else {
	...
	_pdict_ent_notify(n, PDR_ENTRY_ADDED, n->pde_val);
	if (ovp)
		*ovp = NULL;
...
pdict_ent_remove(pdict_t *pd, const char *k, char **ovp)
{
	...
	if (ovp)
		*ovp = (char *)n->pde_val;
		...
		free((void *)n->pde_val); n->pde_val = NULL;
	}
	free((void *)n->pde_key); n->pde_key = NULL;
	pdict_ent_remove_change_listeners(n);
	free(n); n = NULL;
	return 1;
}
...
{
	if (!plist_walk(pd->pd_persistent_listeners,
	    pdict_ent_add_persistent_change_listener_cb, pde)) {
		plist_walk(pd->pd_persistent_listeners,
		    pdict_ent_remove_persistent_change_listener_cb, pde);
		return 0;
	}
	return 1;
}
    ...
    pdict_persistent_listener_t *pdpl)
{
	...
	if (!_pdict_ent_add_change_listener(pde, pdpl->pdpl_l.pdl_notify,
	    pdpl->pdpl_l.pdl_arg))
	    	return 0;
	if (pdpl->pdpl_new)
		pdpl->pdpl_l.pdl_notify(pde->pde_key, pde->pde_val,
		    PDR_CURRENT_VALUE, NULL, pdpl->pdpl_l.pdl_arg);
	return 1;
}
...
pdict_remove_persistent_change_listener(pdict_t *pd, int id)
{
	...
	if (!plist_remove((void *)id, &pd->pd_persistent_listeners,
	    (void **)&pdpl) || !pdpl)
		return 0;
	if (!_pdict_walk_int(pd,
	    pdict_ent_remove_persistent_change_listener_dcb, pdpl))
		return 0;
    ...
    pdl_notify_func_t notify, void *arg)
{
	...
	plist_add((void *)lid, pl, &pd->pd_persistent_listeners);
	...
	if (!_pdict_walk_int(pd,
		...
		plist_remove((void *)lid, &pd->pd_persistent_listeners, NULL);
...
pdict_ent_remove_change_listener_cb(const void *k, const void *v, void *a)
{
	...
	if (l->pdl_notify == (pdl_notify_func_t)arg[0] && l->pdl_arg ==
	    arg[1]) {
		arg[2] = l;
		return 0;
	}
    ...
    void *a)
{
	...
	arg[1] = a;
	arg[2] = NULL;
	plist_walk(pde->pde_listeners, pdict_ent_remove_change_listener_cb,
	    arg);
	if (arg[2]) {
...
_pdict_walk_int(pdict_t *pd, pdict_walk_int_func_t w, void *arg)
{
	...
	return ptree_walk(pd->pd_ents, PTREE_INORDER, pdict_walk_int_cb, args);
}
...
pdict_walk(pdict_t *pd, pdict_walk_func_t w, void *arg)
{
	...
	return ptree_walk(pd->pd_ents, PTREE_INORDER, pdict_walk_cb, args);
}
