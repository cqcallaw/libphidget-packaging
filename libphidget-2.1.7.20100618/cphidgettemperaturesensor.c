...
CPHIDGETDATA(TemperatureSensor)
	...
	{
		phid->AmbientTemperature = PUNK_DBL;
		if (phid->phid.fptrError && phid->AmbientTemperatureError == PFALSE)
		{
			...
			phid->phid.fptrError((CPhidgetHandle)phid, phid->phid.fptrErrorptr, EEPHIDGET_OUTOFRANGE, error_buffer);
	...
	for (i = 0; i<phid->phid.attr.temperaturesensor.numTempInputs; i++)
	{
		...
		{
			phid->Temperature[i] = PUNK_DBL;
			if (phid->phid.fptrError && phid->TemperatureError[i] == PFALSE)
			{
				snprintf(error_buffer,sizeof(error_buffer),
					"Thermocouple %d temperature cannot be calculated because the ambient temperature is unknown.", 
					i);
				phid->phid.fptrError((CPhidgetHandle)phid, phid->phid.fptrErrorptr, EEPHIDGET_OUTOFRANGE, error_buffer);
		...
		else if(Temperature[i] == PUNK_DBL)
		{
			...
			{
				snprintf(error_buffer,sizeof(error_buffer),
					"Thermocouple %d reported an undefined temperature. Probably there is no thermocouple plugged in.", 
		...
		else if(Temperature[i] > phid->temperatureMax[i])
		{
			...
			{
				snprintf(error_buffer,sizeof(error_buffer),
					"Thermocouple %d reported a temperature of %.4fC which is above the maximum of %.4fC.", 
					i, Temperature[i], phid->temperatureMax[i]);
				phid->phid.fptrError((CPhidgetHandle)phid, phid->phid.fptrErrorptr, EEPHIDGET_OUTOFRANGE, error_buffer);
				phid->TemperatureError[i] = PTRUE;
			}
		...
		else if(Temperature[i] < phid->temperatureMin[i])
		{
			...
			{
				snprintf(error_buffer,sizeof(error_buffer),
					"Thermocouple %d reported a temperature of %.4fC which is below the minimum of %.4fC.", 
		...
		{
			phid->Potential[i] = PUNK_DBL;
			if (phid->phid.fptrError && phid->PotentialError[i] == PFALSE)
			{
				snprintf(error_buffer,sizeof(error_buffer),
