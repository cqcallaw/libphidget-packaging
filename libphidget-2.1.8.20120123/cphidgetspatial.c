...
//Zero out calibrations
			...
			// 1044 (1056 Replacement)
			else if (phidG->deviceVersion  >= 400 && phidG->deviceVersion < 500)
			{				//top 2 bits in buffer[0] are packet type
				switch(buffer[0])
				{
					case SPATIAL_PACKET_DATA:
					{
						...
						for(i=0;i<count;i++)
						{
							...
							//ADC ref is 0-3.3V - 50.355uV/bit, gyro zero rate is 1.5V, X/Y: 2.5mV/deg/s Z: 3.572mV/deg/s - these voltages are fixed, non-ratiometric to Vref
							// 1 / 0.000050355 = 19859 (1V)
							// 1.5 * 19859 = 29789
							// 0.0025 * 19859 = 49.6477bits/deg/s
							...
							for(j=0;j<3;j++)
							{
								...
								if(j==1)
									...
									phid->dataBuffer[phid->bufferWritePtr].angularRate[j] = ((double)-(data-29789)) / 70.936348; //reverse X/Z-axis
								else
									phid->dataBuffer[phid->bufferWritePtr].angularRate[j] = ((double)-(data-29789)) / 49.6477; //reverse X/Z-axis
							}
