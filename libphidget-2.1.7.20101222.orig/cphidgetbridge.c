					...
					if(phid->enabledEcho[i] && (buffer[0] & (0x10 << i))) //enabled and have data
					{
						...
						if(err || bridgeVal > phid->bridgeMax[i] || bridgeVal < phid->bridgeMin[i])
						{
							...
							if(!phid->lastOutOfRange[i])
							{
								...
								{
									snprintf(error_buffer,sizeof(error_buffer),"Overrange condition detected on input %d, try lowering the gain.",i);
								...
								{
									snprintf(error_buffer,sizeof(error_buffer),"Underrange condition detected on input %d, try lowering the gain.",i);
									if (phid->phid.fptrError)
										phid->phid.fptrError((CPhidgetHandle)phid, phid->phid.fptrErrorptr, EEPHIDGET_OUTOFRANGE, error_buffer);
								}
							}
						...
						else
						{
							if(phid->lastOutOfRange[i])
							{
								if (phid->phid.fptrError)
								{
									snprintf(error_buffer,sizeof(error_buffer),"Input %d out of range state ended.", i);
									phid->phid.fptrError((CPhidgetHandle)phid, phid->phid.fptrErrorptr, EEPHIDGET_OK, error_buffer);
								}
							}
							phid->lastOutOfRange[i] = PFALSE;

							phid->bridgeValue[i] = bridgeVal;
						}
					}
					else
						phid->bridgeValue[i] = PUNK_DBL;
...
CSETINDEX(Bridge,Gain,CPhidgetBridge_Gain)
	...
	if(CPhidget_statusFlagIsSet(phid->phid.status, PHIDGET_REMOTE_FLAG))
		ADDNETWORKKEYINDEXED(Enabled, "%d", gain);
	else
