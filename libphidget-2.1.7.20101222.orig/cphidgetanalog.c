...
CPHIDGETCLEARVARS(Analog)
	...
	for(i=0;i<ANALOG_MAXOUTPUTS;i++)
	{
		...
		phid->enabled[i] = PUNI_BOOL;
		phid->voltage[i] = PUNI_DBL;
...
CPHIDGETDATA(Analog)
	...
	for (i = 0; i < phid->phid.attr.analog.numAnalogOutputs; i++)
	{
		if(overcurrent[i] && !phid->lastOvercurrent[i])
		{
			...
			{
				snprintf(error_buffer,sizeof(error_buffer),"Output %d is trying to draw > 20mA - possible short circuit.", i);
		...
		else if(!overcurrent[i] && phid->lastOvercurrent[i])
		{
			if (phid->phid.fptrError)
			{
				snprintf(error_buffer,sizeof(error_buffer),"Output %d overcurrent state ended.", i);
	...
	if(tsd && !phid->lastTsd)
	{
		if (phid->phid.fptrError)
		{
			snprintf(error_buffer,sizeof(error_buffer),"Thermal shutdown detected. All outputs have been disabled.", i);
	...
	else if(!tsd && phid->lastTsd)
	{
		if (phid->phid.fptrError)
		{
			snprintf(error_buffer,sizeof(error_buffer),"Thermal shutdown state ended.", i);
			phid->phid.fptrError((CPhidgetHandle)phid, phid->phid.fptrErrorptr, EEPHIDGET_OK, error_buffer);
		}
	}
	phid->lastTsd = tsd;
...
CGETINDEX(Analog,Voltage,double)
	...
	TESTMASGN(voltage[Index], PUNK_DBL)
